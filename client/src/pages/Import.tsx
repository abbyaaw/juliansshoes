...
import { startScan, startIdentify, uploadPhotos } from '../lib/api';
...
export default function Import() {
  ...
  const handleUpload = useCallback(async (files: FileList | File[]) => {
    ...
    if (fileArray.length === 0) {
      addLog('No valid image files selected', 'error');
      return;
    }
    try {
      setUploading(true);
      addLog(`Uploading ${fileArray.length} photo(s)...`);
      const result = await uploadPhotos(fileArray);
      addLog(`Uploaded ${result.uploaded} photo(s)`, 'success');
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-sm text-gray-500 mb-1">
          {uploading ? 'Uploading...' : 'Drag & drop shoe photos here'}
        </p>
        <p className="text-xs text-gray-400 mb-3">JPG, PNG, WebP, HEIC — up to 20 at a time</p>
        <button
          onClick={() => fileInputRef.current?.click()}
          disabled={uploading}
          className="bg-emerald-600 hover:bg-emerald-500 disabled:bg-gray-200 disabled:text-gray-400 text-white font-medium text-sm rounded-lg px-6 py-2.5 transition-colors"
        >
